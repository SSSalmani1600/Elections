    ...
    <div class="max-w-6xl mx-auto py-16 px-4">
      ...
      <div v-else-if="user" class="grid lg:grid-cols-2 gap-8 items-start">
        ...
        <div class="bg-[#111830]/80 border border-white/10 rounded-2xl shadow-xl overflow-hidden lg:sticky lg:top-8">
          ...
          <div v-if="!editMode">
            <!-- User header -->
            <div class="bg-gradient-to-r from-[#ef3054]/20 to-[#d82f4c]/10 p-6 border-b border-white/5">
              <div class="flex items-center gap-4">
                <div class="w-14 h-14 rounded-full bg-gradient-to-br from-[#ef3054] to-[#d82f4c] flex items-center justify-center text-xl font-bold shadow-lg">
              ...
              <div class="flex items-center justify-between p-3 rounded-xl bg-white/5 border border-white/5">
                <div class="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p class="font-medium text-sm">{{ user.isAdmin ? 'Beheerder' : 'Gebruiker' }}</p>
                  </div>
                </div>
                <span v-if="user.isAdmin" class="px-2 py-1 text-xs font-medium bg-emerald-500/20 text-emerald-400 rounded-full border border-emerald-500/30">
                  Admin
                </span>
              </div>
          ...
          <div v-else class="p-6">
            ...
            <form @submit.prevent="saveChanges" class="space-y-4">
              ...
              <div>
                ...
                <input v-model="editUser.username" type="text" required class="w-full p-3 rounded-lg bg-[#0B132B] border border-white/10 focus:border-[#ef3054] focus:outline-none text-white text-sm" />
              </div>

              <div>
                <label class="text-xs font-medium text-gray-400 mb-1 block">E-mailadres</label>
                <input v-model="editUser.email" type="email" required class="w-full p-3 rounded-lg bg-[#0B132B] border border-white/10 focus:border-[#ef3054] focus:outline-none text-white text-sm" />
              </div>

              <div>
                <label class="text-xs font-medium text-gray-400 mb-1 block">Nieuw wachtwoord <span class="text-gray-600">(optioneel)</span></label>
                <input v-model="editUser.newPassword" type="password" placeholder="Minimaal 8 karakters" minlength="8" class="w-full p-3 rounded-lg bg-[#0B132B] border border-white/10 focus:border-[#ef3054] focus:outline-none text-white placeholder-gray-500 text-sm" />
              </div>

              <div v-if="error" class="p-3 rounded-lg bg-red-500/10 border border-red-500/30 text-red-200 text-sm">{{ error }}</div>
              <div v-if="success" class="p-3 rounded-lg bg-emerald-500/10 border border-emerald-500/30 text-emerald-200 text-sm">{{ success }}</div>

              <div class="flex gap-3 pt-2">
                <button type="button" @click="cancelEdit" :disabled="loading" class="flex-1 px-4 py-3 rounded-lg font-medium bg-white/5 border border-white/10 hover:bg-white/10 transition-all text-sm">
                  Annuleren
                </button>
                <button type="submit" :disabled="loading || !currentPassword" class="flex-1 px-4 py-3 rounded-lg font-medium bg-gradient-to-r from-[#ef3054] to-[#d82f4c] text-white transition-all disabled:opacity-50 text-sm">
