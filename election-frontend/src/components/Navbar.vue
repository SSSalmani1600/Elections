...
const isStemwijzerActive = computed(() =>
  route.path.startsWith('/stemwijzer')
)
...
<template>
  <nav
    class="h-[100px] bg-background flex items-center justify-between px-6 sticky top-0 shadow-lg z-100"
  >
    ...
    <div class="hidden lg:flex items-center justify-center gap-8">
      ...
      <router-link to="/stemwijzer" :class="{ 'router-link-custom-active': isStemwijzerActive }" class="nav-link">Stemwijzer</router-link>
    </div>
    ...
    >
      ...
      >
        <div class="w-full h-full p-6 flex flex-col gap-6">
          ...
          <router-link to="/calendar" class="nav-link" @click="toggleMenu"
            >Verkiezingskalender</router-link
          >
          <router-link to="/stemwijzer" class="nav-link" @click="toggleMenu"
            >Stemwijzer</router-link
          >
          <template v-if="!user">
